...
import cn.zmdx.kaka.locker.meiwen.widget.BaseScrollView;
...
public class SingleImageBox implements IPandoraBox, View.OnClickListener {
    ...
    private void initViews() {
        mEntireView = (ViewGroup) LayoutInflater.from(mContext).inflate(
                Res.layout.pandora_box_single_image, null);
        mScrollView = (BaseScrollView) mEntireView.findViewById(Res.id.scrollView);
        // mScrollView.setOnScrollListener(listener)
        // mScrollView.setOnTouchListener(new
        // ScrollTouchListener(mScrollListener));
        mSingleImgView = (ImageView) mEntireView.findViewById(Res.id.single_img);
        setImageViewSize(mSingleImgView);
        mDescView = (TextView) mEntireView.findViewById(Res.id.desc);
        mDescView.getPaint().setFakeBoldText(true);
        mFromPlatformText = (TextView) mEntireView.findViewById(Res.id.from_platform_text);
        mImageNewsContent = (TextView) mEntireView.findViewById(Res.id.image_news_content);
        // mBackBtn =
        // mEntireView.findViewById(Res.id.pandora_box_single_back_btn);
