...
package cn.zmdx.kaka.locker.meiwen.battery;
...
public class BatteryView extends View {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                setContentDescription(context
                        .getString(Res.string.accessibility_battery_level, level));
                postInvalidate();
                if (null != mCallBack) {
    ...
    public BatteryView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray levels = res.obtainTypedArray(Res.array.batterymeter_color_levels);
        TypedArray colors = res.obtainTypedArray(Res.array.batterymeter_color_values);
        ...
        mWarningString = context.getString(Res.string.battery_meter_very_low_overlay_symbol);

        mFramePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mFramePaint.setColor(res.getColor(Res.color.batterymeter_frame_color));
        ...
        mBoltPaint.setColor(res.getColor(Res.color.batterymeter_bolt_color));
        mBoltPoints = loadBoltPoints(res);
        setLayerType(View.LAYER_TYPE_SOFTWARE, null);
    }

    private static float[] loadBoltPoints(Resources res) {
        final int[] pts = res.getIntArray(Res.array.batterymeter_bolt_points);
        int maxX = 0, maxY = 0;
    ...
    private int getBatteryChargeColor(int percent) {
        if (percent <= BATTERY_LOW) {
            mChargeColor = getResources().getColor(Res.color.batterymeter_charge_color_low);
        } else {
            mChargeColor = getResources().getColor(Res.color.batterymeter_charge_color_high);
        }
        return mChargeColor;
    }
    ...
    private int getBatteryRegularColor(int percent) {
        if (percent <= BATTERY_LOW) {
            ...
            mChargeColor = getResources().getColor(Res.color.batterymeter_regular_color);
        }
        return mChargeColor;
    }
