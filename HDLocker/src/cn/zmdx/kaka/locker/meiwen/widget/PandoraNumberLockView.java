...
public class PandoraNumberLockView extends LinearLayout {
    ...
    private void init() {
        mRootView = LayoutInflater.from(mContext).inflate(Res.layout.pandora_number_lock, null);
        addView(mRootView);
        mKeyboardView = (KeyboardView) mRootView.findViewById(Res.id.pandora_keyboard_view);
        mKeyboardView.setKeyboard(new Keyboard(mContext, Res.layout.pandora_number_lock_content));
        mKeyboardView.setPreviewEnabled(false);
        mKeyboardView.setOnKeyboardActionListener(mKeyboardListener);
        mPromptTextView = (TypefaceTextView) mRootView.findViewById(Res.id.number_lock_prompt);
        mDownPassWordMark = (LinearLayout) mRootView.findViewById(Res.id.pandora_number_layout);
        mNumberLayout = (LinearLayout) findViewById(Res.id.pandora_number_layout);
        mNumberOne = (ImageView) mRootView.findViewById(Res.id.pandora_number_one);
        mNumberTwo = (ImageView) mRootView.findViewById(Res.id.pandora_number_two);
        mNumberThree = (ImageView) mRootView.findViewById(Res.id.pandora_number_three);
        mNumberFour = (ImageView) mRootView.findViewById(Res.id.pandora_number_four);
    }
    ...
    private void createVerifyFailAnimations() {
        int displacementPX = (int) mContext.getResources().getDimension(
                Res.dimen.pandora_number_lock_translation_x_displacement);
    ...
    protected void showPasswordStringBuffer(boolean isNeedAnimation) {
        switch (mPassword.toString().length()) {
            case 0:
                mNumberOne.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberTwo.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberThree.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberFour.setImageResource(Res.drawable.pandora_number_lock_line);
                break;
            case 1:
                mNumberOne.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberTwo.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberThree.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberFour.setImageResource(Res.drawable.pandora_number_lock_line);
                if (isNeedAnimation) {
            ...
            case 2:
                mNumberOne.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberTwo.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberThree.setImageResource(Res.drawable.pandora_number_lock_line);
                mNumberFour.setImageResource(Res.drawable.pandora_number_lock_line);
                if (isNeedAnimation) {
            ...
            case 3:
                mNumberOne.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberTwo.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberThree.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberFour.setImageResource(Res.drawable.pandora_number_lock_line);
                if (isNeedAnimation) {
            ...
            case 4:
                ...
                mNumberTwo.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberThree.setImageResource(Res.drawable.pandora_number_lock_point);
                mNumberFour.setImageResource(Res.drawable.pandora_number_lock_point);
                if (isNeedAnimation) {
    ...
    protected void openNumberLock() {
        if (isDetectedOnce(onPatternDetectedTimes)) {
            if (!checkNumberLock()) {
                setPromptString(mContext.getResources().getString(
                        Res.string.number_lock_confirmation_fail_prompt));
        ...
        setPromptString(mContext.getResources().getString(Res.string.number_lock_confirmation_prompt));
    ...
    protected void closeNumberLock() {
        if (checkNumberLock()) {
            ...
            setPromptString(mContext.getResources().getString(Res.string.number_lock_verify_fail));
            clearPasswordStringBuffer(true);
        }
