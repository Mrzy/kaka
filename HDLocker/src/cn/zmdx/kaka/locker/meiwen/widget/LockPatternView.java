...
public class LockPatternView extends View {
    ...
    public LockPatternView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, Res.styleable.LockPatternView);

        final String aspect = a.getString(Res.styleable.LockPatternView_aspect);
        ...
        mPathWidth = getResources().getDimensionPixelSize(Res.dimen.lock_pattern_dot_line_width);
        mPathPaint.setStrokeWidth(mPathWidth);

        mDotSize = getResources().getDimensionPixelSize(Res.dimen.lock_pattern_dot_size);
        mDotSizeActivated = getResources().getDimensionPixelSize(
                Res.dimen.lock_pattern_dot_size_activated);
    ...
    private void notifyCellAdded() {
        sendAccessEvent(Res.string.lockscreen_access_pattern_cell_added);
    ...
    private void notifyPatternStarted() {
        sendAccessEvent(Res.string.lockscreen_access_pattern_start);
        if (mOnPatternListener != null) {
    ...
    private void notifyPatternDetected() {
        sendAccessEvent(Res.string.lockscreen_access_pattern_detected);
        if (mOnPatternListener != null) {
    ...
    private void notifyPatternCleared() {
        sendAccessEvent(Res.string.lockscreen_access_pattern_cleared);
        if (mOnPatternListener != null) {
    ...
    private void startLineEndAnimation(final CellState state, final float startX,
            final float startY, final float targetX, final float targetY) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawPath) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (i != 0) {
                    ...
                    if (state.lineEndX != Float.MIN_VALUE && state.lineEndY != Float.MIN_VALUE) {
                        ...
                        mPathShader = new LinearGradient(lastX, lastY, state.lineEndX,
                                state.lineEndY, new int[] {
                                        lastCellState.color, state.color
                                }, new float[] {
                                        0, 1.0f
                                }, TileMode.MIRROR);
                    } else {
                        currentPath.lineTo(centerX, centerY);
                        mPathShader = new LinearGradient(lastX, lastY, centerX, centerY, new int[] {
                                lastCellState.color, state.color
                        }, new float[] {
                                0, 1.0f
                        }, TileMode.MIRROR);
            ...
            if ((mPatternInProgress || mPatternDisplayMode == DisplayMode.Animate) && anyCircles) {
                ...
                mPathPaint.setAlpha((int) (calculateLastSegmentAlpha(mInProgressX, mInProgressY,
                        lastX, lastY) * 255f));
                canvas.drawPath(currentPath, mPathPaint);
            }
        }
