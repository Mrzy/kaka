...
public class ProgressBarMaterial extends CustomView {
    ...
    protected void setAttributes(AttributeSet attrs) {
        ...
        setMinimumHeight(BaseInfoHelper.dip2px(getContext(), 3));
    ...
    private void drawFirstAnimation(Canvas canvas) {
        if (radius1 < getWidth() / 2) {
            ...
            if (cont >= 50) {
                ...
                radius2 = (radius2 >= getWidth() / 2 - BaseInfoHelper.dip2px(getContext(), 4)) ? (float) getWidth()
                        / 2 - BaseInfoHelper.dip2px(getContext(), 4)
                        : radius2 + 1;
            }
            temp.drawCircle(getWidth() / 2, getHeight() / 2, radius2, transparentPaint);
            canvas.drawBitmap(bitmap, 0, 0, new Paint());
            if (radius2 >= getWidth() / 2 - BaseInfoHelper.dip2px(getContext(), 4))
                cont++;
