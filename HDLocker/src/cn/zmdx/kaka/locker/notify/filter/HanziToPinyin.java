...
public class HanziToPinyin {
    ...
    private static final String LOCAL_ZH_HANS_CN = "zh_HANS_CN";
    ...
    public static HanziToPinyin getInstance() {
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA) || locale[i].toString().equals(LOCAL_ZH_HANS_CN)) {
                    // Do self validation just once.
                    if (DEBUG) {
                        Log.i(TAG, "Self validation. Result: " + doSelfValidation());
                    }
    ...
    private Token getToken(char character) {
        ...
        if (offset < 0) {
            ...
            while (begin <= end) {
            	offset = Math.round(begin/2.0f + end/2.0f);
