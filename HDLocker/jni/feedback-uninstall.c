...
jstring Java_com_yepstudio_android_library_feedback_uninstall_AppUninstall_onUninstall(
	...
	//LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &b_IS_COPY), (*env)->GetStringUTFChars(env, action, &b_IS_COPY));
	//LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &b_IS_COPY), (*env)->GetStringUTFChars(env, data, &b_IS_COPY));
